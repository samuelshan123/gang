...
} from '../../utils/push-notification/notification-helper';
      ...
      try {
        ...
        if (gangsResponse.status === 200) {
          ...
          if (!lastUpdatedEpochs.length) {
            payload = gangsResponse.gangs.map(gang => ({
          ...
          if (payload.length > 0) {
            ...
            const messagesResponse = await postData('gang/messages', {
              gangs: payload,
            });
              ...
              realm.write(() => {
                messagesResponse.messages.forEach(message => {
                  ...
                  if (gang) {
                    ...
                    // Use object shorthand to assign the last_message
                    gang.last_message = {
                      id,
                      content,
                      contentType,
                  ...
                  let lastUpdate = realm.objectForPrimaryKey(
                    'GangMessageLastUpdated',
                    message.gangId,
                  );
                  if (lastUpdate) {
        ...
        actions={[
          {
            ...
            onPress: () =>
              navigation.navigate('Create Gang', {
                user,
              }),
          },
          {
